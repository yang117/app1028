...
//        recyclerView.setAdapter(adapter);
        ...
        switch (view.getId()) {
            case android.R.id.text1:
                if (token != null) {
                    ...
                    Toast.makeText(MainActivity2.this, "Authenticate first.", Toast.LENGTH_LONG).show();
                }
                break;
        ...
        public void onResponse(Call<ListWrapper<Question>> call, Response<ListWrapper<Question>> response) {
            if (response.isSuccessful()) {
                ...
                ArrayAdapter<Question> arrayAdapter = new ArrayAdapter<Question>(
                        MainActivity2.this, android.R.layout.simple_spinner_dropdown_item, questions.items);
                questionsSpinner.setAdapter(arrayAdapter);
            } else {
    ...
    Callback<ResponseBody> upvoteCallback = new Callback<ResponseBody>() {
        ...
        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
            if (response.isSuccessful()) {
                Toast.makeText(MainActivity2.this, "Upvote successful", Toast.LENGTH_LONG).show();
            } else {
                Log.d("QuestionsCallback", "Code: " + response.code() + " Message: " + response.message());
                Toast.makeText(MainActivity2.this, "You already upvoted this answer", Toast.LENGTH_LONG).show();
            }
        }
